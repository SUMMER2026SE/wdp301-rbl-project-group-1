...
export function TutorRequestCard({ request, onApply }: TutorRequestCardProps) {
  ...
  const createdAtFormatted = format(new Date(request.createdAt), "dd/MM/yyyy", {
    locale: vi,
  });

  const scheduleText =
    request.scheduleRules && request.scheduleRules.length > 0
      ? request.scheduleRules
          .map((r) => `${formatDay(r.dayOfWeek)} (${r.startTime}-${r.endTime})`)
          .join(", ")
      : "Chưa có lịch cụ thể";
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="pt-2 flex flex-col xl:flex-row xl:items-end justify-between gap-6">
            <div className="flex flex-wrap items-center gap-y-3 gap-x-6 text-sm font-medium text-muted-foreground">
              ...
              {request.grade?.name && (
                <div className="flex items-center gap-2">
                  <div className="p-1.5 rounded-md bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  {request.budget
                    ? `${request.budget.toLocaleString("vi-VN")} VND / buổi`
                    : "Thỏa thuận"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="truncate max-w-[250px]" title={scheduleText}>
                  {scheduleText}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="truncate max-w-[200px]">
                  {request.mode === "ONLINE" ? "Học trực tuyến" : "Học tại nhà"}
                </span>
              </div>
