...
import { X, UserCircle, BookOpen, GraduationCap, DollarSign, Clock, FileText, Check, XCircle } from "lucide-react";
...
const statusMap: Record<TutorApplication["status"], { label: string; className: string }> = {
  PENDING: {
    label: "Đang chờ",
...
export function TutorApprovalDetailModal({ application, trigger }: TutorApprovalDetailModalProps) {
  const [approve, { isLoading: isApproving }] = useApproveTutorApplicationMutation();
  const [reject, { isLoading: isRejecting }] = useRejectTutorApplicationMutation();
    ...
    >
      <div className="p-6 md:p-8 overflow-y-auto max-h-[80vh]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div>
            <Badge className={`h-7 rounded-full border px-3 text-sm ${statusMeta.className}`}>
              {statusMeta.label}
            </Badge>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-6 shadow-sm">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground mb-1">{application.specialization || "Gia sư"}</h3>
            <p className="text-sm font-medium text-muted-foreground mb-3">{application.email}</p>
            <div className="flex flex-wrap gap-2">
              <Badge variant="outline" className="bg-muted/30">
                Kinh nghiệm: {application.experience ? `${application.experience} năm` : "Chưa cập nhật"}
              </Badge>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-muted/30 border border-border/50 rounded-xl p-4 flex gap-3">
              ...
              <div>
                ...
                <p className="font-semibold text-sm text-foreground">{application.education || "—"}</p>
              </div>
            </div>
            ...
            <div className="bg-muted/30 border border-border/50 rounded-xl p-4 flex gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground mb-1">Học phí mong muốn</p>
                <p className="font-semibold text-sm text-foreground">
                  {application.pricePerHour 
                    ? new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(application.pricePerHour) + "/giờ" 
                    : "—"}
                </p>
              </div>
            </div>
          ...
          <div>
            <h4 className="text-sm font-medium text-muted-foreground mb-3 uppercase tracking-wider">Chuyên môn & Cấp bậc</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-muted/30 border border-border/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <BookOpen className="size-4 text-muted-foreground" />
                  <p className="text-xs text-muted-foreground font-medium">Môn học đăng ký</p>
                </div>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="bg-muted/30 border border-border/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">Lớp/Cấp học</p>
                </div>
                <div className="flex flex-wrap gap-2">
          ...
          <div>
            <h4 className="text-sm font-medium text-muted-foreground mb-3 uppercase tracking-wider">Giới thiệu bản thân</h4>
            <div className="bg-muted/30 border border-border/50 rounded-xl p-4 relative overflow-hidden">
              <div className="z-10 relative">
                {application.bio ? (
                  <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{application.bio}</p>
                ) : (
                  <p className="text-sm text-muted-foreground italic">Không có thông tin giới thiệu.</p>
                )}
              </div>
            </div>
          </div>
          ...
          {application.files && application.files.length > 0 && (
            <>
              <Separator className="bg-border/50" />
              <div>
                <h4 className="text-sm font-medium text-muted-foreground mb-3 uppercase tracking-wider">Tệp đính kèm</h4>
                <div className="flex flex-col gap-2">
                  {application.files.map((file, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-3 rounded-lg border border-border/50 bg-card hover:bg-muted/30 transition-colors">
                      ...
                      <a href={file} target="_blank" rel="noreferrer" className="text-sm font-medium text-foreground hover:underline truncate">
                        {file.split('/').pop() || `Tài liệu đính kèm ${idx + 1}`}
                      </a>
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-end gap-3 pt-6 border-t border-border/50">
          <DialogClose asChild>
            <Button variant="outline" className="rounded-xl px-6 h-11 border-border/50" disabled={isBusy}>
              Đóng
            </Button>
          </DialogClose>
