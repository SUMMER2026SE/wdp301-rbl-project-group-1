...
import { Dialog, DialogContent, DialogTitle, DialogTrigger } from '@/src/shared/components/ui/dialog';
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen} modal={false}>
      ...
      <DialogContent
        ...
        [&_[data-slot=dialog-close]]:p-4
        [&_[data-slot=dialog-close]]:rounded-xl
      ...
      >
        <nav className="flex flex-col text-center gap-4 mt-10 px-6">
          {menu?.map(({ key, trigger, href, content }) => (
            <div key={key} className="w-full text-left">
              {href ? (
                ...
                <div className="text-3xl font-semibold mb-5 hover:underline pb-4">{trigger}</div>
              )}
          ...
          {children && (
            <div className="mt-6 flex flex-col items-center gap-4 w-full">{children}</div>
          )}
        </nav>
      </DialogContent>
    </Dialog>
